...
saveRDS(d, "wealth_and_health.rds")
```
...
Region(Geographic) (E)(e_regiongeo)

In which geographic region these countreis are located?

![](images/4da6583dd278334d552c28f0f728464.png)

![](images/ee16f48b5573a83490a71e16b91e4e3.png)

## Democracy Indicators: Boix et al. 2013

![](images/1aa288d5b2818f61619af63a439c8e1.png){width="595"}
...
d_pop_overlap <- d |> select(country_text_id, year, starts_with("population_")) |> 
  drop_na()
print(d_pop_overlap, n = 3) #Print first three rows of d_pop_overlap
unique(d_pop_overlap$year) #Range of years in the data frame
cor(d_pop_overlap$population_ClioInfra, d_pop_overlap$population_WorldBank) 
#Correlation between population_ClioInfra and population_WorldBank (strength and diretion of the linear relationship)
```
...
```{r}
dir.create("data")
saveRDS(d, "data/wealth_and_health.rds")
```
...
#Add information and contexts to the plot to make it more visually appealing and informative (geom(),labs(),title(),captio())
```
...
dir.create("figures")
saveRDS(p_all, "figures/wealth_and_health_all.rds")
# Save plot as a PDF file in your folder
ggsave(filename = "figures/welath_and_health_all.pdf", 
